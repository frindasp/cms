...
const backupSchema = z.object({
  ...
  databaseType: z.enum(["TIDB", "SUPABASE"]),
...
export function BackupForm({ initialData, onSubmit, isLoading }: BackupFormProps) {
  ...
  const form = useForm<BackupFormValues>({
    ...
    defaultValues: {
      ...
      databaseType: initialData?.databaseType || "TIDB",
  ...
  const onTestConnection = async () => {
    ...
    const toastId = toast.loading("Testing Connection", { 
      description: <CountdownProgress initialCount={20} text={`Connecting to ${values.databaseType} server...`} /> 
    });
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (type === "TIDB") form.setValue("port", 3306);
    if (type === "SUPABASE") form.setValue("port", 5432);
  };
